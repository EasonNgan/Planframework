...
```sql
select b.* from UnitPlanPattern a inner join UnitplanpatternCenunit b on a.PatternID = b.PatternID where EstateName_c in (N'又一居', N'德福花園', N'爾巒', N'東環', N'天賦海灣')

-- Map cenunit which have G in y_axis
with t1 as (select b.c_estate, b.c_phase, b.c_property, b.cblgcode,
  a.cuntcode,a.x_axis,a.y_axis from three..cenunit a inner join three..cenbldg b on a.cblgcode = b.cblgcode where c_estate in (N'又一居', N'德福花園',) and a.y_axis like '%G%')
  ...
  where EstateName_c in (N'又一居', N'德福花園')

-- Map cenunit which y_axis only contain number
;with t1 as (select b.c_estate, b.c_phase, b.c_property, b.cblgcode,
  a.cuntcode,a.x_axis,a.y_axis from three..cenunit a inner join three..cenbldg b on a.cblgcode = b.cblgcode where c_estate in (N'又一居', N'德福花園'))
  --insert into UnitplanpatternCenunit(patternid,cuntcode)
  select distinct patternid,cuntcode from UnitPlanPattern a inner join t1 on a.EstateName_c = t1.c_estate and isnull(a.PhaseName_c,'') = isnull(t1.c_phase,'') 
  and a.BuildingName_c = t1.c_property
...
```sql
select * from FloorPlanPattern a inner join FloorplanpatternCenbldg b on a.PatternID = b.PatternID where EstateName_c in (N'又一居', N'德福花園')
order by a.patternid 
;with t1 as (select b.c_estate, b.c_phase, b.c_property, b.cblgcode
 from three..cenbldg b where c_estate in (N'又一居', N'德福花園'))
